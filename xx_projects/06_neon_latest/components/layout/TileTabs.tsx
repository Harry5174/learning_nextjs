...
export default function TileTabs({
    ...
    teamMembers.length ? (
      <section>
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          {teamMembers.map((p) => {
            // Handle both data structures: featuredImage (from tiles) and peopleFields.headshot (from deals)
            const img = p.featuredImage?.node || p.peopleFields?.headshot?.node;
            return (
  ...
  const DealCard = ({ d }: { d: Deal }) => {
    ...
    const linkPath = isDealPage ? `/News/deals/${d.slug}` : `/tile/${d.slug}`;
    
    return (
      <a href={linkPath} className="flex gap-4 p-4 border rounded-lg hover:shadow-md transition">
        {img?.sourceUrl && (
          <img
            src={img.sourceUrl}
